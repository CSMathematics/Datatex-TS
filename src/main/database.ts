...
export class DatabaseManager {
  ...
  private seedDefaults() {
    ...
    if (fieldCount.count === 0) {
      ...
      this.db
        .prepare('INSERT INTO Fields (Id, Name) VALUES (?, ?)')
        .run('field-universal', 'Universal')
    }

    // Έλεγχος και δημιουργία Default FileType
    const typeCount = this.db.prepare('SELECT count(*) as count FROM FileTypes').get() as {
      count: number
    }
    if (typeCount.count === 0) {
      console.log('Seeding default FileType...')
      this.db
        .prepare('INSERT INTO FileTypes (Id, Name, FolderName) VALUES (?, ?, ?)')
        .run('type-tex', 'LaTeX File', 'tex_files')
    }
  }
  ...
  public createFile(fileData: { title: string; content: string; type: string; chapter: string }) {
    ...
    const createTx = this.db.transaction(() => {
      ...
      const existingChapter = this.db!.prepare('SELECT Id FROM Chapters WHERE Name = ?').get(
        chapterName
      ...
      if (existingChapter) {
        ...
        chapterId = uuidv4()
        this.db!.prepare('INSERT INTO Chapters (Id, Name, Field) VALUES (?, ?, ?)').run(
          chapterId,
          chapterName,
          'field-universal'
        ) // Default field
      ...
      this.db!.prepare('INSERT INTO Chapters_per_File (File_Id, Chapter_Id) VALUES (?, ?)').run(
        fileId,
        chapterId
      )
    })

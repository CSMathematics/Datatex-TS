...
interface DBFile { id: number; title: string; content: string; type: string; chapter: string; }
interface TabFile extends DBFile { isDirty?: boolean; originalContent?: string; }
...
const App: React.FC = () => {
  ...
  const loadFilesFromDB = async () => {
    try {
      if (!window.api) {
         ...
         const mockFiles = [{ id: 1, title: 'Demo.tex', content: '% Demo', type: 'tex', chapter: 'Demo' }];
         setDbFiles(mockFiles); processFiles(mockFiles); return;
      }
      const files = await window.api.getFiles();
      setDbFiles(files);
      processFiles(files);
    } catch (error: any) { message.error("Failed to load files"); }
  };
  ...
  const processFiles = (files: DBFile[]) => {
      ...
      files.forEach(file => {
        ...
        if (!treeMap[chapterName]) treeMap[chapterName] = { 
            title: chapterName, 
            key: `chapter-${chapterName}`, 
            icon: <span role="img" aria-label="folder" className="anticon">📁</span>, 
            children: [] 
        };
        treeMap[chapterName].children.push({ 
            title: file.title, 
            key: String(file.id), 
            icon: <span role="img" aria-label="file" className="anticon">📄</span>, 
            isLeaf: true, 
            data: file 
        });
      });
  ...
  const insertTextAtCursor = (text: string) => {
      if (!editorRef.current || !activeFileId) { message.warning("Open a file first!"); return; }
  ...
  const handleEditorChange = (value: string | undefined) => {
      ...
      setOpenFiles(prev => prev.map(f => String(f.id) === activeFileId ? { ...f, content: value, isDirty: value !== f.originalContent } : f));
  };
  ...
  const saveCurrentFile = async () => {
      ...
      try {
          ...
          setOpenFiles(prev => prev.map(f => f.id === currentFile.id ? { ...f, isDirty: false, originalContent: f.content } : f));
          setDbFiles(prev => prev.map(f => f.id === currentFile.id ? { ...f, content: currentFile.content } : f));
          message.success("Saved!"); setStatusText("Saved");
      } catch (err) { message.error("Failed to save"); setStatusText("Error saving"); }
  };

  const handleCreateFile = async (values: { title: string; chapter: string }) => {
      try {
          const newFilePayload = { title: values.title.endsWith('.tex') ? values.title : `${values.title}.tex`, chapter: values.chapter, type: 'tex', content: '' };
      ...
      } catch (err) { message.error("Failed to create"); }
  };

  const handleDeleteFile = async () => {
      if (!activeFileId) return;
      Modal.confirm({
          title: 'Delete File', content: 'Are you sure?', okText: 'Delete', okButtonProps: { danger: true },
          onOk: async () => {
              const id = Number(activeFileId);
              if (window.api) await window.api.deleteFile(id);
              else { const filtered = dbFiles.filter(f => f.id !== id); setDbFiles(filtered); processFiles(filtered); }
              closeTab(activeFileId); message.success("Deleted");
          }
      });
  };
  ...
  const onNodeSelect = (selectedKeys: React.Key[]) => {
    ...
    if (file) {
        if (!openFiles.find(f => f.id === file.id)) setOpenFiles([...openFiles, { ...file, originalContent: file.content, isDirty: false }]);
  ...
  const closeTab = (targetKey: string | any) => {
      ...
      if (activeFileId === String(targetId)) setActiveFileId(newOpenFiles.length > 0 ? String(newOpenFiles[newOpenFiles.length - 1].id) : null);
  };
      ...
      <Layout style={{ height: '100vh', width: '100vw', overflow: 'hidden' }}>
        ...
        <Layout style={{ flex: 1, minWidth: 0, background: '#1e1e1e' }}> 
          <div style={{ height: 35, background: '#252526', display: 'flex', overflowX: 'auto' }}>
             {openFiles.length === 0 ? <div style={{ padding: '8px 16px', color: '#888', fontSize: 12 }}>No files open</div> : (
                <Tabs type="editable-card" activeKey={activeFileId || undefined} onChange={setActiveFileId} onEdit={closeTab} hideAdd size="small"
                    items={openFiles.map(f => ({ label: <span>{f.title}{f.isDirty && ' ●'}</span>, key: String(f.id), closable: true }))}
                    tabBarStyle={{ margin: 0, height: 35, borderBottom: '1px solid #303030' }}
                />
             )}
          </div>
          
          <Header style={{ padding: '0 16px', background: '#1e1e1e', height: 40, display: 'flex', alignItems: 'center', borderBottom: `1px solid ${colorBorder}` }}>
            <span style={{ color: '#aaa', fontSize: 12 }}>{activeFileId && openFiles.find(f => String(f.id) === activeFileId)?.title}</span>
            <div style={{ flex: 1 }} />
            {activeFileId && <Tooltip title="Delete"><Button type="text" danger icon={<DeleteOutlined />} onClick={handleDeleteFile} /></Tooltip>}
            <Tooltip title="Save"><Button type="text" icon={<SaveOutlined />} onClick={saveCurrentFile} style={{ color: '#ccc' }} /></Tooltip>
            <Button type="primary" size="small" icon={<PlayCircleOutlined />} style={{ background: '#237804', marginLeft: 8 }}>Compile</Button>
          </Header>
            ...
            <Content style={{ height: '100%', background: '#1f1f1f' }}>
                {activeFileId ? (
                    <Editor 
                        ...
                        options={{ minimap: { enabled: true }, fontSize: 14, fontFamily: "'Fira Code', monospace" }} 
                    />
                ) : <Empty description="Select a file" style={{ marginTop: 100 }} image={Empty.PRESENTED_IMAGE_SIMPLE} />}
            </Content>
            <Sider width="40%" theme="light" style={{ borderLeft: `1px solid ${colorBorder}`, background: '#f0f0f0' }}>
                <div style={{ height: 32, background: '#e0e0e0', display: 'flex', alignItems: 'center', paddingLeft: 8, fontSize: 11, fontWeight: 'bold', color: '#555' }}>PDF PREVIEW</div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}><Empty description="No PDF" /></div>
            </Sider>
          </Layout>
        </Layout>
        ...
        <PreambleWizard 
            ...
            onInsert={(code) => { insertTextAtCursor(code); setIsPreambleOpen(false); }} 
        />

        <TableWizard 
            open={isTableOpen} 
            onCancel={() => setIsTableOpen(false)} 
            onInsert={(code) => { insertTextAtCursor(code); setIsTableOpen(false); }} 
        />
